...
for filename in os.listdir(DATA_DIR):
    ...
    jpeg_cropped = jpeg_file.resize((IM_WIDTH, IM_HEIGHT))
    jpeg_cropped.save(os.path.join(DATA_DIR, filename))
    # Import RAW RGB data
    # raw = np.reshape(jpeg_cropped.getdata(), (IM_WIDTH, IM_HEIGHT, 3)).astype(float)
    # Convert RGB to LAB
